...
class BerryfolioTestCase(unittest.TestCase):
    ...
    def upload(self, pid, title, description, tag_1, tag_2, tag_3):
    ...
    def modify(self, fid, pid, title, description, tag_1, tag_2, tag_3):
        return self.app.post('/portfolio', data=dict(
            type=u"Attribute",
            fid=fid,
            pid=pid,
    ...
    def test_upload_modify_directory(self):
        ...
        assert "欢迎进入Berryfolio" in rv.data
        # 上传文件
        rv = self.upload(u'3', u'测试图片', u'描述', u"采风", u"", u"3t34")
        assert "上传成功" in rv.data
        # 修改属性
        data_1 = self.query_fid(3).data
        rv = self.modify(u"3", u"7", u"我的新照片", u"修改过的描述", u"", u"采风", u"sggse")
